...
# Root directory of Mask R-CNN and helper tools
...
def init_mask_rcnn():
    ...
    model_path = "final_model.h5"  # Trained model weights
    ...
    model.keras_model._make_predict_function() # fixesTensor Tensor("mrcnn_detection/Reshape_1:0", shape=(1, 100, 6), dtype=float32) is not an element of this graph.
    return model



# Converts matplotlib to PIL images stored in memory instead of secondary storage
# to avoid unneccasry W operations on the disk
def fig2img(fig):
...
# will take an RGB image (converted from BGR) and the trained model
def pred_img(img, model):
    ...
    # PIL png object that needs to be converted to np array
    mask_img = np.array(mask_img)
    # Once converted to np array change from RGB to BGR
    # to be proccessed using cv2 (still np array but color channels are changed)
